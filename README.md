...
| `gate_rejections_total` | Counter | `model`, `backend` (the reported rejector's backend on 429 exhaustion, or the sentinel `none` on 502 exhaustion — same semantics as the `gate_requests_total` `backend` label), `reason` (`exceeds_tier` \| `auto_exceeds_headroom` \| `all_backends_failed`), `tier_kv_pct` (the governing tier's `kv_pct` percentage as a string, e.g. `80`, when the tiered layer is the rejector; `0` otherwise) | Rejected requests by refusal reason: `exceeds_tier` = the request's estimated context exceeds the governing tier's max_context (the tiered layer rejected); `auto_exceeds_headroom` = the always-on autoconfig headroom layer rejected; `all_backends_failed` = every candidate transport-failed (the 502-exhaustion path). |
